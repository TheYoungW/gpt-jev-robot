...
[首轮对照试验已完成](../experiments/results/v2_pilot_01/REPORT.md)：A/B 单步进展均为 13.9%，未发现动作效果提升；结构化输入相较本试验的逐字段文本少用 36.4% token。还发现 release 条件会阻止盘外抓空后的张开恢复。该已知限制在当前版本保留，实验没有通过临时修改规则来抬高分数。
